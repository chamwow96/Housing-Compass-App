"""
Streamlit App V5 Enhanced — Local Housing Policy Assessment Tool
Adds: Excel Spreadsheet Download, UI/UX Improvements, Dynamic Filtering
"""
...
if page == "🏘️ Policy Assessment Tool":
    ...
    *Research the level of preemptions that may exist for your community.*
    ...
    challenge = st.radio(
        ...
        ["Supply Shortage (not enough housing units, especially affordable ones)", "Affordability Crisis (rents/prices rising faster than incomes, displacement pressures)", "Housing Quality (aging stock, deep need for rehab, aging homeowner community)", "Multiple Crises (two or more equal issues)"]
    )
    ...
    if st.button("📊 Generate Policy Recommendations"):
        ...
        if priority_list:
            ...
            for p in priority_list:
                st.markdown(f"**{p}**")
                guidance_text = GUIDANCE.get(p, "")
                st.markdown(
                    f"<div style='background-color:#f0f8ff; padding:10px; border-radius:5px; font-size:16px'>{guidance_text}</div>",
                    unsafe_allow_html=True
                )
        ...
        if remaining:
            ...
            for r in remaining:
                st.markdown(f"**{r}**")  # <-- use r here
                guidance_text = GUIDANCE.get(r, "")  # <-- and here
                st.markdown(
                    f"<div style='background-color:#f0f8ff; padding:10px; border-radius:5px; font-size:16px'>{guidance_text}</div>",
                    unsafe_allow_html=True
                )
        ...
        csv_filename = f'policy_recommendation_v5_{datetime.today().strftime("%Y%m%d")}.csv'
        ...
        # -------------------------
        # Load and allow download of Policy Tool Examples
        # -------------------------
        policy_file = "Policy Tool Examples.xlsx"

        if os.path.exists(policy_file):
            st.divider()
            st.subheader("📌 Policy Tool Examples Library")
            st.markdown(
                "Download the full spreadsheet of recommended tools and resources for your community."
            )

            # Download button for full Excel file
            with open(policy_file, "rb") as f:
                st.download_button(
                    "Download Policy Tool Examples (Excel)",
                    data=f,
                    file_name=policy_file,
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
        else:
            st.warning(f"Policy Tool Examples file not found: `{policy_file}`. Please add it to the app directory.")
